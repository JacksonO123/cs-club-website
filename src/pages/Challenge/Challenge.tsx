...
export default function Challenge({ user }: Props) {
  ...
  const submit = () => {
    ...
    challenge.tests.forEach((test: any, i: number) => {
      ...
      //console.log(code);
      fetch(
        `http://cs-club-api.eba-dmppb5a2.us-west-2.elasticbeanstalk.com/submit/${code}/${language}/${test.input}`,
        {
          //?code='${code}'&language='${language}'&input='${test.input}'`, {
          method: 'post',
          // mode: "no-cors",
          // body: JSON.stringify({
          //     code,
        ...
        .then((data) => {
          ...
          if (data.error) {
            ...
            (data.output != test.output || test.output + '\n' == data.output) &&
            solved
          ) {
            setStatus(
            ...
            // if (solved) {
            setStatus(
              `Congratulations! Passed ${challenge.tests.length}/${challenge.tests.length} test cases!`
            );
